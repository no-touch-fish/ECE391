...
volatile int32_t rtc_flag = 0; //1 means interrupt should happen, 0 mean waiting
...
void rtc_handler () {
    ...
    int32_t interrupt_interval = real_frequency / device_frequency;//initialize the interrupt interval
    cli();
    rtc_counter++;//increment counter when the handler is called
    //set rtc interrupt flag if exact one interrupt interval is passed
    if (0 == (rtc_counter % interrupt_interval)) rtc_flag = 1;//1 means ready to return back to the process
...
int32_t rtc_open (const uint8_t* filename) {
    rtc_flag = 0;// clear the rtc flag, step in a new process
    device_frequency = 2;//reset the frequency to 2
...
int32_t rtc_read (int32_t fd, void* buf, int32_t nbytes) {
    while (1) {//keep waiting until the flag is set and it's ready to return to the process
        if (1 == rtc_flag) break;//exit the loop when flag is set to 1
    }
    rtc_flag = 0;//clear the flag after exit the spin loop
...
int32_t rtc_write (int32_t fd, const void* buf, int32_t nbytes) {
    ...
    if ((frequency_desired > 0) && ((frequency_desired & (frequency_desired - 1)) == 0) && frequency_desired <= real_frequency && frequency_desired >= 2){
        device_frequency = frequency_desired;//set the virtualized frequency
